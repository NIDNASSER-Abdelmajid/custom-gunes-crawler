...
function figureOut(flags) {
  ...
  if (
    crawlConfig.forceOverwrite === undefined ||
    flags.forceOverwrite !== undefined
  ) {
    crawlConfig.forceOverwrite = Boolean(flags.forceOverwrite);
  }
  if (
    crawlConfig.filterOutFirstParty === undefined ||
    flags.only3p !== undefined
  ) {
  ...
  if (
    crawlConfig.disableAntiBot === undefined ||
    flags.disableAntiBot !== undefined
  ) {
  ...
  if (flags.batchSize) {
    crawlConfig.batchSize = Number(flags.batchSize);
  }
  ...
  if (flags.url) {
    ...
    // Support lists that are either plain URLs per line OR CSV with header.
    // Requirement: ignore the first line and, for each subsequent line, split by comma and take index 0 as the URL.
    const rawLines = fs.readFileSync(flags.inputList).toString().split(/\r?\n/);
    const lines = rawLines.filter((l) => l && l.trim().length > 0);
    const withoutHeader = lines.length > 0 ? lines.slice(1) : [];
    cliUrls = withoutHeader
      .map((l) => {
  ...
  if (cliUrls) {
    ...
    if (crawlConfig.urls) {
      ...
      const urlsWithConfig = crawlConfig.urls.filter(
        (i) => typeof i !== "string"
      );
      crawlConfig.urls = cliUrls.map(
        (url) => urlsWithConfig.find((i) => i.url === url) || url
      );
    } else {
