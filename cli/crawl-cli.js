...
const {
  getCollectorIds,
  createCollector,
} = require("../helpers/collectorsList");
...
program
  ...
  .option(
    "-d, --data-collectors <list>",
    `comma separated list of data collectors: ${getCollectorIds().join(
      ", "
    )} (all by default)`
  )
  .option(
    "--reporters <list>",
    `comma separated list of reporters: ${getReporterIds().join(", ")}`
  )
  .option(
    "-l, --log-path <path>",
    "instructs reporters where all logs should be written to"
  )
  .option(
    "-v, --verbose",
  ...
  .option(
    "-c, --crawlers <number>",
    "overwrite the default number of concurent crawlers"
  )
  ...
  .option(
    ...
    "optional 2 letter region code. Used for metadata only."
  )
  .option(
    "-a, --disable-anti-bot",
    "disable anti bot detection protections injected to every frame"
  )
  .option("--config <path>", "crawl configuration file")
  .option(
    "--autoconsent-action <action>",
    "dismiss cookie popups. Possible values: optout, optin"
  )
  .option(
  ...
  const urls = inputUrls.filter((item) => {
    ...
    if (forceOverwrite !== true) {
      ...
      if (fs.existsSync(outputFile)) {
        log(
          chalk.yellow(
            `Skipping "${urlString}" because output file already exists.`
          )
        );
        return false;
      }
...
if (!config.urls || !config.output) {
  ...
  if (fs.existsSync(config.output)) {
    if (metadataFileExists(config.output) && !config.forceOverwrite) {
      ...
      console.log(
        chalk.red("Output folder already exists and contains metadata file."),
        "Use -f to overwrite."
      );
      process.exit(1);
    }
